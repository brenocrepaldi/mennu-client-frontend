...
import { OrderSuccessIncentive } from './components/order-success-incentive';

type SuccessViewProps = ReturnType<typeof useSuccessModel>;

export function SuccessView(props: SuccessViewProps) {
	const { navigate, restaurant, isFromLogin, isFromRegister, isFromOrder, isAuthenticated } = props;
	...
	return (
		<Page>
			<div className="w-full bg-secondary flex flex-col items-center gap-6 px-4 py-6 text-center shadow-bottom">
				...
				{/* If it's an order success, show incentive component */}
				{isFromOrder ? (
					<OrderSuccessIncentive
						isAuthenticated={isAuthenticated}
						onCreateAccount={() => void navigate('/register')}
						onViewOrder={() => void navigate('/orders')}
					/>
				) : (
					// Original success message for login/register
					<>
